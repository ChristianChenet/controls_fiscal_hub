...
final class CTeConnector extends AbstractFiscalCollector
{
    ...
    public function collectByAccessKey(string $accessKey): array
    {
        ...
        $requestXml = $this->buildConsChCTeXml($companyCnpj, $accessKey);
        $this->storage->appendLog(
            'collector_cte_key_request.log',
        ...
        $this->storage->appendLog('collector_cte_key.log', 'CT-e [' . $company['cnpj'] . '] chave=' . $accessKey . ' cStat=' . $result['cStat'] . ' mensagem=' . $message);
        ...
        return [
            ...
            'message' => 'CT-e chave ' . $accessKey . ': ' . ($message ?: 'sem retorno'),
        ];
    }
...
XML;
    ...
    private function consultaProtocoloUrl(string $accessKey): string
    {
        ...
        throw new \RuntimeException('URL de consulta de protocolo CT-e nao configurada para a UF da chave ' . $uf . '. Configure CTE_CONSULTA_PROTOCOLO_URL no ambiente.');
    }
    ...
    private function parseProtocolStatusResponse(string $soap): array
    {
        ...
        foreach ($events ?: [] as $event) {
            ...
            if ($isCancellation && ($isAccepted || $eventStatus === '')) {
                return [
                    'cStat' => '101',
                    'xMotivo' => $text('.//*[local-name()="infEvento"]/*[local-name()="xMotivo"]', $event) ?: 'Cancelamento localizado na consulta de protocolo CT-e.',
                    'nProt' => $text('.//*[local-name()="infEvento"]/*[local-name()="nProt"]', $event),
                    'dhRecbto' => $text('.//*[local-name()="infEvento"]/*[local-name()="dhRegEvento"]', $event),
    ...
    private function buildConsChCTeXml(string $cnpj, string $accessKey): string
    {
        $tpAmb = (string)$this->config['sefaz_environment'];
        $cUFAutor = (string)$this->config['sefaz_uf_author'];
        return '<distDFeInt xmlns="http://www.portalfiscal.inf.br/cte" versao="1.00"><tpAmb>'
            . $tpAmb
            . '</tpAmb><cUFAutor>'
            . $cUFAutor
            . '</cUFAutor><CNPJ>'
            . $cnpj
            . '</CNPJ><consChCTe><chCTe>'
            . $accessKey
            . '</chCTe></consChCTe></distDFeInt>';
    }
}
