...
abstract class AbstractFiscalCollector implements CollectorInterface
{
    ...
    protected function extractReturnXml(string $soapResponse): string
    {
        ...
        $xp = new DOMXPath($dom);
        $nodes = $xp->query('//*[local-name()="nfeDistDFeInteresseResult" or local-name()="cteDistDFeInteresseResult" or local-name()="nfeRecepcaoEventoResult" or local-name()="nfeRecepcaoEventoNFResult" or local-name()="nfeConsultaNFResult" or local-name()="cteConsultaCTResult"]');
        ...
        $nodes = $xp->query('//*[local-name()="retDistDFeInt" or local-name()="retEnvEvento" or local-name()="retConsSitNFe" or local-name()="retConsSitCTe"]');
