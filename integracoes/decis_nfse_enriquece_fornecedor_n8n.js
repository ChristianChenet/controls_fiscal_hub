...
async function fetchCnpjWsData(cnpj) {
...
const fillAddress = (json, data) => {
  const tipoLogradouro = clean(data.tipoLogradouro || data.descricao_tipo_de_logradouro);
  const logradouro = clean(data.logradouro);
  const endereco = first(
    tipoLogradouro && logradouro ? `${tipoLogradouro} ${logradouro}` : '',
    logradouro,
  );
  const telefone = first(
    data.telefone,
    data.ddd && data.telefone1 ? `${data.ddd} ${data.telefone1}` : '',
    data.ddd1 && data.telefone1 ? `${data.ddd1}${data.telefone1}` : '',
    data.ddd_telefone_1,
    data.ddd_telefone_2,
    data.ddd_fax,
  );
  ...
  json.CGJURIDICA_CNPJ = first(json.CGJURIDICA_CNPJ, data.cnpj);
...
const normalizeCnpjWs = (data) => {
  ...
  const inscricaoAtiva = Array.isArray(estabelecimento.inscricoes_estaduais)
    ? estabelecimento.inscricoes_estaduais.find((item) => item?.ativo && clean(item?.inscricao_estadual) !== '')
      || estabelecimento.inscricoes_estaduais.find((item) => clean(item?.inscricao_estadual) !== '')
    : null;
  return {
    cnpj: estabelecimento.cnpj || data?.cnpj_raiz,
    ...
    codigo_municipio_ibge: estabelecimento.cidade?.ibge_id,
    cnae_fiscal: estabelecimento.atividade_principal?.id,
    data_inicio_atividade: estabelecimento.data_inicio_atividade,
    inscricoes_estaduais: inscricaoAtiva ? [inscricaoAtiva] : [],
  };
};
  ...
  if (cnpj.length === 14 && needsData) {
    try {
      ...
      if (result.ok && result.data) {
        fillAddress(json, {...result.data, cnpj});
        json.CNPJ_ENRIQUECIDO_ORIGEM = 'BRASILAPI';
      }

      const addressStillIncomplete = [
        json.CGENDERECO_LOGRADOURO,
        json.CGENDERECO_NUMERO,
        json.CGENDERECO_BAIRRO,
        json.CGENDERECO_CEP,
        json.CGENDERECO_CIDADE,
        json.CGENDERECO_UF,
        json.CGENDERECO_CODIGOMUNICIPIO,
      ].some((value) => clean(value) === '');

      if (addressStillIncomplete) {
        const cnpjWsResult = await fetchCnpjWsData(cnpj);
        json.CNPJ_WS_CONSULTADO = 'SIM';
        json.CNPJ_WS_STATUS = String(cnpjWsResult.status || '');
        if (cnpjWsResult.ok && cnpjWsResult.data) {
          fillAddress(json, normalizeCnpjWs(cnpjWsResult.data));
          json.CNPJ_ENRIQUECIDO_ORIGEM = clean(json.CNPJ_ENRIQUECIDO_ORIGEM)
            ? `${json.CNPJ_ENRIQUECIDO_ORIGEM}+CNPJ_WS`
            : 'CNPJ_WS';
        }
      }
    } catch (error) {
